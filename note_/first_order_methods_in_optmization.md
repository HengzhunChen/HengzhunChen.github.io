...
Let $f: \mathbb{E} \rightarrow (-\infty, \infty]$ be a proper and convex function. Suppose that $X\subseteq \text{int}(\text{dom}f)$. Consider the following two claims:
...
$$
x^* \in \argmin \{f(x):x\in \mathbb{E}\} \Longleftrightarrow 0 \in \partial f(x^*)
$$ 
...
$$
\partial f(x) = \argmax_{\tilde{y}\in\mathbb{E}^*}  \{ \langle x, \tilde{y} \rangle - f^*(\tilde{y})\}
$$

and
$$
\partial f^*(y) = \argmax_{\tilde{x}\in\mathbb{E}}  \{ \langle y, \tilde{x} \rangle - f(\tilde{x})\}
$$
...
THM: Conjugate Correspondence Theorem
Let $\sigma > 0$. Then
...
$$
\text{prox}_f(x) = \argmin_{u\in \mathbb{E}} \left\{ f(u) + \frac{1}{2}\|u-x\|^2 \right\} \quad \text{for any } x \in \mathbb{E}.
$$
...
Let $C\subseteq \mathbb{E}$ be nonempty. Then $\text{prox}_{\delta_C}(x)=P_C(x)=\argmin_{u\in C}\|u-x\|^2$.
...
Dynamic Step size: move $\sum_{n=0}^k t_n$ into right hand side through $f_{\text{best}}^k:= \min_{n=0, 1, \cdots, k} f(x^n)$ to make it decay to 0
...
$$
x^{k+1} = \argmin_{x\in C} \left\{ f(x^k) + \langle f'(x^k), x-x^k \rangle + \frac{1}{2t_k}\|x-x^k\|^2 \right\}
$$
...
$$
\begin{align*}
x^{k+1} &= \argmin_{x\in C} \left\{ f(x^k) + \langle f'(x^k), x-x^k \rangle + \frac{1}{t_k}B(x-x^k) \right\} \\
&\argmin_{x\in C} \left\{ \langle t_kf'(x^k) - \nabla \omega(x^k) , x \rangle  + \omega(x) \right\}
\end{align*}
$$
...
Instead of linearizing both $f$ and $g$, we will linearize $f$ and keep $g$ as it is. If we pick $f'(x^k)\in \partial f(x^k)$ and set
$$
\begin{align*}
x^{k+1} &= \argmin_{x} \left\{ \langle f'(x^k), x \rangle + g(x^k) + \frac{1}{t_k}B(x-x^k) \right\} \\
&=\argmin_{x} \left\{ \langle t_kf'(x^k) - \nabla \omega(x^k) , x \rangle + t_kg(x)  + \omega(x) \right\}
\end{align*}
$$
...
$$
\begin{align*}
x^{k+1} &= \argmin_{x\in \mathbb{E}} \left\{ f(x^k) + \langle \nabla f(x^k), x-x^k \rangle + g(x) + \frac{1}{2t_k}\|x-x^k\|^2 \right\}  \\
&= \argmin_{x\in\mathbb{E}} \left\{ t_k g(x) + \frac{1}{2}\|x - (x^k - t_k \nabla f(x^k))\|^2 \right\} \\
...
$$
\Lambda_a = \argmax_{v\in\mathbb{E}} \{\langle a, v \rangle : \|v\|\leq 1\}
$$  
...
- Dual Proximal Gradient (primal representation)
    ...
    Remark. The primal sequence $x^k=\argmax_x \{ \langle x, \mathcal{A}^T(y^k) \rangle - f(x)\}$ are actually not necessarily feasible w.r.t. the primal problem  

- Fast Dual Proximal Gradient (dual/primal representation)

- Dual Block proximal Gradient Method

    Apply the dual method to problem $\min_{x\in \mathbb{E}} \left\{ f(x) + \sum_{i=1}^p g_i(x) \right\}$
...
- compute $p^k \in \argmax_{p\in C} \langle p, \nabla f(x^k) \rangle$
...
- compute $p^k \in \argmax_{p\in \mathbb{E}} \left\{ \langle p, \nabla f(x^k) \rangle + g(p) \right\}$
- choose $t_k\in [0, 1]$ and set $x^{k+1}= x^k + t_k (p^k - x^k) = (1 - t_k) x^k + t_k p^k$.
...
$$
x_i^{k+1} \in \argmin_{x_i\in\mathbb{E}_i} F(x_1^{k+1}, \cdots, x_{i-1}^{k+1}, x_i, x_{i+1}^k, \cdots, x_p^k)
$$
...
$$
x^*_i \in \argmin_{y\in\mathbb{E}_i} {f(x^* + \mathcal{U}_i (y-x^*_i)) + g_i(y)},
$$
...
$$
y^{k+1} = \argmin_{y\in\mathbb{R}^m} \left\{ h_1^*(-A^Ty) + h_2^*(-B^Ty) + \langle c, y \rangle + \frac{1}{2\rho} \|y-y^k\|^2 \right\}
$$
...
$$
\begin{align*}
x^{k+1} &\in \argmin_{x} \left\{ h_1(x) + \frac{\rho}{2} \|Ax+Bz^k - c + \frac{1}{\rho} y^k \|^2 \right\} \\
z^{k+1} &\in \argmin_{z} \left\{ h_2(z) + \frac{\rho}{2} \|Ax^{k+1} + Bz - c + \frac{1}{\rho} y^k \|^2 \right\} \\
y^{k+1} &= y^k + \rho(Ax^{k+1} + Bz^{k+1} - c) \\
\end{align*}
$$
...
$$
\begin{align*}
x^{k+1} &\in \argmin_{x} \left\{ h_1(x) + \frac{\rho}{2} \|Ax+Bz^k - c + \frac{1}{\rho} y^k \|^2 \right\} + \frac{1}{2} \|x-x^k\|_G^2 \\
z^{k+1} &\in \argmin_{z} \left\{ h_2(z) + \frac{\rho}{2} \|Ax^{k+1} + Bz - c + \frac{1}{\rho} y^k \|^2 + \frac{1}{2} \|z-z^k\|^2_Q \right\} \\
y^{k+1} &= y^k + \rho(Ax^{k+1} + Bz^{k+1} - c) \\
\end{align*}
$$
